...
function getItemData(skLayer,skParent){
  ...
  return getTextData(skLayer,skParent);
    ...
    translateRelativeToSymbolInstance(items,instancePosition,masterAbsolutePosition);
    return items;
  }
  return;
}
...
function translateRelativeToSymbolInstance(items,instancePosition,masterAbsolutePosition){
  for (var i=0;i<items.length;i++){
    var item=items[i];
    var absolutePosition = item.absolutePosition;
    ...
    if(item.type=="group"){
      translateRelativeToSymbolInstance(item.items,instancePosition,masterAbsolutePosition);
    }
  }
}
...
function getTextRanges(nsAttributedString, rangeToProcess){
  var ranges  = [];
  ...
  var rangeData = {
    ...
    'text_color':getJustinmindColorFromUIColor(foregroundColor),
  ...
  if(nextRangeStart<nsAttributedString.string().length()){
    ...
    var nextRanges = getTextRanges(nsAttributedString,nextRangeToProcess);
...
function getTextData(skText,skParent){
  ...
  var ranges = getTextRanges(nsAttributedString,fullRange);
