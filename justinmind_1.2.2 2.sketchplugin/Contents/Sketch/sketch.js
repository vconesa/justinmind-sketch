...
function getRotatedBBoxForGroups(position,frame,angle){
  if(angle!=0){
    var rad = toRadians(angle);
    var center = {'x':Math.round(position.x+(frame.width/2)),'y':Math.round(position.y+(frame.height/2))};
    var topLeft = rotatePointAroundPoint({'x':position.x,'y':position.y},center,rad);
...
function calculateAbsolutePosition(skLayer){
  ...
  if( (isText(skLayer) && !enableExportTextAsImage) || isTranslatableShape(skLayer)){
    var absoluteRect = skLayer.absoluteRect();
...
function getBoundingBox(items){
  ...
  for (var i=0;i<items.length;i++){
    ...
    if(item.type=='line'){
      ...
      if(item.shapeInitialPoint.x < minX)
      	minX=item.shapeInitialPoint.x;
      if(item.shapeInitialPoint.y < minY)
      	minY=item.shapeInitialPoint.y;
      if(item.shapeEndPoint.x < minX)
      	minX=item.shapeEndPoint.x;
      if(item.shapeEndPoint.y < minY)
      	minY=item.shapeEndPoint.y;
      if(item.shapeInitialPoint.x > maxX)
      	maxX=item.shapeInitialPoint.x;
      if(item.shapeInitialPoint.y > maxY)
      	maxY=item.shapeInitialPoint.y;
      if(item.shapeEndPoint.x > maxX)
      	maxX=item.shapeEndPoint.x;
      if(item.shapeEndPoint.y > maxY)
      	maxY=item.shapeEndPoint.y;

    }else{

    	var position = item.absolutePosition;
    	var angle = item.rotation;
    	if(angle!=null && angle!=0 && item.type!="group"){
    		position = getRotatedBBoxForGroups(position,item.size,angle);
    	}
    	var size= item.size;
    	if(position.x < minX)
    		minX = position.x;
...
function isTranslatableShape(skShape){
    var shape = sketch.fromNative(skShape);
    var style = shape.style;

     if(style.fills.length>1) return false;
     if(style.shadows.length>1) return false;
     if(style.borders.length>1) return false;
     if(isAncestorMasked(skShape)) return false;
     if(shape.shapeType == ShapePath.ShapeType.Rectangle ){
       if(shape.points.length==4){

      var radius1 = shape.points[0].cornerRadius;
      var radius2 = shape.points[1].cornerRadius;
      var radius3 = shape.points[2].cornerRadius;
      var radius4 = shape.points[3].cornerRadius;
    ...
    if(style.fills && style.fills.length)
    {
        ...
        if(fill.enabled)
        {
        if(fill.fillType.toString() != "Color")
  			return false;
        }
    }
...
*/
    if(isAncestorMasked(skShape))
       return getSVGLayerData(skShape,skParent);
    if(!isTranslatableShape(skShape))
       return getSVGLayerData(skShape,skParent);
    if(shape.shapeType == ShapePath.ShapeType.Rectangle )
    ...
    if(shape.shapeType == ShapePath.ShapeType.Custom && shape.points.length==2){
        if(shape.points[0].pointType !="Straight" || shape.points[0].pointType !="Straight")
           return getSVGLayerData(skShape,skParent);
...
function getLineData(skShape,skParent, shape)
{
    ...
    */
    ...
    var pos2 = calculateAbsolutePosition(skShape);
    ...
    'name':shape.name,
      ...
      'visible':!shape.hidden,
        ...
        'border':jimBorder,
        'fill':jimFill,
        'shapeInitialPoint':{'x':pos2.x,'y':pos2.y},
        'shapeEndPoint':{'x':pos2.x+a,'y':pos2.y+b}
    };
}
...
*/
    ...
    var type = "rectangle";
    if(shape.name.startsWith("in_")) type="InputText";
    return {
      'objectID':shape.id,
      'type':type,
    'name':shape.name,
